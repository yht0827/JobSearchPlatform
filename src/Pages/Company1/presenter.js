...
const Company1 = ({ action, changeAction, steps, changeNextStep, stepReset, current, handleLoader, isLoaded }) => (
    <>
        <Header title="Company1" link="/company1" onClick={() => { stepReset(); handleLoader(true); }} bg="white" fg={oc.blue[7]} gr1={oc.blue[6]} gr2={oc.indigo[7]}>
        ...
        <Section>
            ...
            { action === "apply" && <FormWrapper bg={oc.indigo[8]} BoxWid="1000px" BoxHei="750px" LogoName="Application">
                <ApplyTemPlate questions={questions[0]} btcolor="blue" icolor={oc.blue[7]} subcolor="teal" bg={oc.blue[3]} steps={steps} changeNextStep={changeNextStep} current={current} stepReset={stepReset} handleLoader={handleLoader} isLoaded={isLoaded} />
            </FormWrapper> }
        </Section>
    </>
);
